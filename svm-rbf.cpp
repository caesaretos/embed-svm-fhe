...
const string DATA_FOLDER = "../data-kernel-model-rbf/";

int main() {

    cout << "FHE EMBED SVM RBF Kernel started ... !\n\n";

    #if NATIVEINT == 128
        std::cout << "OpenFHE int size 128-bit\n";
    #else
        std::cout << "OpenFHE int size 64-bit\n";
    #endif

    uint32_t n = 8; // SVM vectors dimensions (# of predictors)
    
    // polynomial kernel parameters
    vector<double> gamma_param = read_double_data_from_file(DATA_FOLDER + "gamma.txt");
    double gamma = gamma_param[0];
    std::cout << "gamma: " << gamma << "\n";
    // uint32_t degree = 3;
    vector<vector<double>> support_vectors = read_2d_matrix_from_file(DATA_FOLDER + "support_vectors_rbf.txt");
    ...
    // read the data
    vector<double> dual_coeffs = read_double_data_from_file(DATA_FOLDER + "dual_coeff_rbf.txt");
    vector<double> bias = read_double_data_from_file(DATA_FOLDER + "intercept_rbf.txt");
    resize_double_vector(bias, n);
    vector<double> x = read_double_data_from_file(DATA_FOLDER + "xtest_rbf.txt");
    vector<double> y_ground_truth = read_double_data_from_file(DATA_FOLDER + "ytest_rbf.txt");
    ...
    cout << "batch size: " << next_power_of_2(batchSize*n_SVs) << "\n";
    ...
    cc->EvalRotateKeyGen(keys.secretKey, {0, 1, 2, 4, 8}); // powers of two upto n
    ...
    //     cout << res;
    //     std::vector<double> res_vals = res->GetRealPackedValue();
    //     auto minmax = std::minmax_element(res_vals.begin(), res_vals.end());
    ...
    // bias
    bias.resize(next_power_of_2(n*n_SVs), 0.0);
    Plaintext pt_bias = cc->MakeCKKSPackedPlaintext(bias);
    // dual coeffs
    vector<double> dual_coeffs_vec(next_power_of_2(n*n_SVs), 0.0);
    ...
    double lowerBound = -200.0, upperBound = 2.0;
    uint32_t polyDegree = 59; // 13, 27, 59, 119, 247, 495;
    ...
    auto diff = cc->EvalSub(ct_x, pt_support_vectors);
    // decrypt_and_print(diff, "diff");
    diff = cc->EvalSquare(diff);
    // decrypt_and_print(diff, "square");
    auto sum = total_sum(diff, batchSize);
    // decrypt_and_print(sum, "sum");
    sum = cc->EvalMult(sum, pt_gamma); // gamma already has the negative sign
    // decrypt_and_print(sum, "sum * -gamma");
    auto ct_out = cc->EvalChebyshevFunction([](double y) -> double { return std::exp(y); }, sum, lowerBound,
                                        upperBound, polyDegree);
    // decrypt_and_print(ct_out, "exp");
